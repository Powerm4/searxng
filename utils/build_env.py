...
brand_env = f'utils{sep}brand.env'
...
for name, option in name_val:
    if os.environ.get(name, _unset) is not _unset:
        del os.environ[name]
...
print(
    f"build {brand_env} (settings from: {os.environ['SEARXNG_SETTINGS_PATH']})"
)
sys.path.insert(0, repo_root)

with open(repo_root + sep + brand_env, 'w', encoding='utf-8') as f:
    for name, option in name_val:
        print(f"export {name}='{_env(option)}'", file=f)
